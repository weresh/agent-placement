...
function PreviousTasks() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      setWork(completedTasks.map(task => ({
        caseid: task.id,
  ...
  return (
    <main className='bg-[#D9D9D9] min-h-screen p-3'>
      <div className='bg-white w-full p-3 border border-black rounded-md'>
        <h1 className='text-left text-3xl md:text-2xl my-5 font-bold'>PREVIOUS TASKS</h1>
        <div className='border border-black my-2 overflow-x-auto rounded-lg'>
          <table className="w-full table-auto">
            ...
            <tbody>
              ...
              {work.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-4 py-2 border border-black text-center">No Previous Completed Work</td>
                </tr>
              )}
              {work?.map(works => (
                <tr key={works.caseid}>
                  ...
                  <td className="px-4 py-2 border border-black">
                    <p className="border border-green-600 whitespace-nowrap p-1 rounded-xl">
                      {works.status}
                    </p>
                  </td>
