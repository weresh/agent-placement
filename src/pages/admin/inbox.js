...
function Inbox() {
  const work= [
    { agentid: 'AG001', caseid: 55780, reporttitle: 'check allocation', status: 'Allocated', category:'Field', receivedon:'2024-04-15 08:11' },
    { agentid: 'AG002', caseid: 55781, reporttitle: 'home visitation', status: 'Rejected', category:'Desk', receivedon: '2024-04-16 09:11' },
    { agentid: 'AG004', caseid: 55782, reporttitle: 'home inspection', status: 'Allocated', category:'Field', receivedon: '2024-04-18 11:11' },
    { agentid: 'AG003', caseid: 55783, reporttitle: 'check allocation', status: 'Rejected', category:'Desk', receivedon: '2024-04-19 12:11' },
    { agentid: 'AG001', caseid: 55784, reporttitle:'home inspection', status: 'Allocated', category:'Field', receivedon: '2024-04-21 14:11' },
    { agentid: 'AG002', caseid: 55785, reporttitle: 'home inspection', status: 'Rejected', category:'Field', receivedon:'2024-04-22 15:11' },
    { agentid: 'AG003', caseid: 55786, reporttitle: 'check allocation', status: 'Allocated', category:'Desk', receivedon: '2024-04-23 16:11' },
    { agentid: 'AG004', caseid: 55787, reporttitle:'home visitation', status: 'Allocated', category:'Field', receivedon: '2024-04-24 17:11' }
  ];
    ...
    <main className='bg-[#D9D9D9] min-h-screen p-3 '>
      <div className='bg-white  w-full p-3 border border-black  rounded-md'>
        ...
        <div className='flex gap-3 font-bold ml-2'>
          ...
          >ALL</p>
          <p 
            className={`pb-2 mx-4 md:px-7 cursor-pointer ${filter === 'Allocated' ? 'border-b border-red-700' : ''}`}
            onClick={() => setFilter('Allocated')}
          >ALLOCATED</p>
          <p 
            className={`pb-2 mx-4 md:px-7  cursor-pointer ${filter === 'Rejected' ? 'border-b border-red-700' : ''}`}
		...
		<th className="px-4 py-2 text-start border border-gray-400"> REPORT TITLE</th>
                ...
                <th className="px-4 py-2 text-start border border-gray-400">RECEIVED ON</th>
              </tr>
            </thead>
            <tbody>
		  ...
		  <td className="px-4 py-2 border border-black">{cases.agentid}</td>
                  <td className="px-4 py-2 border border-black">{cases.caseid}</td>
                  <td className="px-4 py-2 border border-black">{cases.reporttitle}</td>
		              <td className="px-4 py-2 border border-black">{cases.category}</td>
                  <td className="px-4 py-2 border border-black">
                    <p className={`border p-1 rounded-xl ${cases.status === 'Allocated' ? 'border-green-600' : 'border-red-600'}`}>
                      {cases.status}
                    </p>
                  </td>
                  <td className="px-4 py-2 border border-black">{cases.receivedon}</td>
                </tr>
              ))}
            </tbody>
          </table>
